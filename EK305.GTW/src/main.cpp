...
#define EK305CAN_ECU01_ENABLED true
...
// pinos para a barra de leds no volante
...
#include <EK305SD.h>
#include <EK305CAN.h> // importa biblioteca de funções para a CAN (alto nivel)
#include <EEPROM.h>
...
// U8GLIB_ST7920_128X64_1X(sck, mosi, cs [, reset]) // Usar a partir do EK305

// U8GLIB_ST7920_128X64_1X u8g(LCD_D0, LCD_D1, LCD_D2, LCD_D3, LCD_D4, LCD_D5, LCD_D6, LCD_D7, LCD_EN, LCD_RS, LCD_RW, LCD_RE); // SERIAL - USO PARA DISPLAY NO PAINEL USANDO MUITOS CABOS - LEGADO (EK305)

U8GLIB_ST7920_128X64_1X u8g(LCD_EN, LCD_RW, LCD_RS); // SPI - USO PARA O DISPLAY NO VOLANTE - EK305
...
void setupSensors()
{
  sensors[SENSOR_SPEED_ID] = Sensor("Speed", "Km/h", SENSOR_SPEED_ID, EK305CAN_ID_ADDRESS_ECU04, EK305CAN_SPEED_FREQUENCY);
  sensors[SENSOR_ROTATION_ID] = Sensor("Mot. Rotation", "rpm", SENSOR_ROTATION_ID, EK305CAN_ID_ADDRESS_ECU02, EK305CAN_RPM_FREQUENCY);
  sensors[SENSOR_MOTOR_TEMPERATURE_ID] = Sensor("Mot. Temperature", "oC", SENSOR_MOTOR_TEMPERATURE_ID, EK305CAN_ID_ADDRESS_ECU03, EK305CAN_TEMPERATURE_FREQUENCY);
  sensors[SENSOR_GEAR_ID] = Sensor("Gear", "", SENSOR_GEAR_ID, EK305CAN_ID_ADDRESS_ECU03, EK305CAN_GEAR_POSITION_FREQUENCY);
  sensors[SENSOR_GPS_LATITUDE_ID] = Sensor("Latitude", "o", SENSOR_GPS_LATITUDE_ID, EK305CAN_ID_ADDRESS_GTW, EK305CAN_GPS_FREQUENCY);
  sensors[SENSOR_GPS_LONGITUDE_ID] = Sensor("Longitude", "o", SENSOR_GPS_LONGITUDE_ID, EK305CAN_ID_ADDRESS_GTW, EK305CAN_GPS_FREQUENCY);
  sensors[SENSOR_GPS_ALTITUDE_ID] = Sensor("Altitude", "o", SENSOR_GPS_ALTITUDE_ID, EK305CAN_ID_ADDRESS_GTW, EK305CAN_GPS_FREQUENCY);
  sensors[SENSOR_GPS_FIX_ID] = Sensor("GPS Fix", "o", SENSOR_GPS_FIX_ID, EK305CAN_ID_ADDRESS_GTW, EK305CAN_GPS_FREQUENCY);
  sensors[SENSOR_GPS_HDOP_ID] = Sensor("GPS Fix", "", SENSOR_GPS_HDOP_ID, EK305CAN_ID_ADDRESS_GTW, EK305CAN_GPS_FREQUENCY);
  sensors[SENSOR_VOLTAGE_BATTERY_ID] = Sensor("Main Supply", "V", SENSOR_VOLTAGE_BATTERY_ID, EK305CAN_ID_ADDRESS_GTW, EK305CAN_VOLTAGE_FREQUENCY);
  sensors[SENSOR_MAP_ID] = Sensor("MAP", "kPa", SENSOR_MAP_ID, EK305CAN_ID_ADDRESS_ECU02, EK305CAN_MAP_FREQUENCY);
  sensors[SENSOR_AIR_INTAKE_TEMPERATURE_ID] = Sensor("Air Temp.", "°C", SENSOR_AIR_INTAKE_TEMPERATURE_ID, EK305CAN_ID_ADDRESS_ECU02, EK305CAN_AIR_TEMP_FREQUENCY);
  sensors[SENSOR_WATER_TEMPERATURE_ID] = Sensor("Water Temp.", "°C", SENSOR_WATER_TEMPERATURE_ID, EK305CAN_ID_ADDRESS_ECU02, EK305CAN_WATER_TEMP_FREQUENCY);
  sensors[SENSOR_TPS_ID] = Sensor("TPS", "%", SENSOR_TPS_ID, EK305CAN_ID_ADDRESS_ECU02, EK305CAN_TPS_FREQUENCY);
  sensors[SENSOR_LAMBDA_ID] = Sensor("Lambda", "", SENSOR_LAMBDA_ID, EK305CAN_ID_ADDRESS_ECU02, EK305CAN_LAMBDA_FREQUENCY);
  sensors[SENSOR_OIL_PRESSURE_ID] = Sensor("Oil Pressure", "kPa", SENSOR_OIL_PRESSURE_ID, EK305CAN_ID_ADDRESS_ECU04, EK305CAN_OIL_PRESSURE_FREQUENCY);
  sensors[SENSOR_OIL_TEMPERATURE_ID] = Sensor("Oil Temp.", "°C", SENSOR_OIL_TEMPERATURE_ID, EK305CAN_ID_ADDRESS_ECU04, EK305CAN_OIL_TEMP_FREQUENCY);
  sensors[SENSOR_REAR_LEFT_SUSPENSION_ID] = Sensor("RL Susp.", "°", SENSOR_REAR_LEFT_SUSPENSION_ID, EK305CAN_ID_ADDRESS_ECU04, EK305CAN_SUSP_REAR_FREQUENCY);
  sensors[SENSOR_REAR_RIGHT_SUSPENSION_ID] = Sensor("RR Susp", "°", SENSOR_REAR_RIGHT_SUSPENSION_ID, EK305CAN_ID_ADDRESS_ECU04, EK305CAN_SUSP_REAR_FREQUENCY);
  sensors[SENSOR_FRONT_LEFT_SUSPENSION_ID] = Sensor("FL Susp", "°", SENSOR_FRONT_LEFT_SUSPENSION_ID, EK305CAN_ID_ADDRESS_ECU01, EK305CAN_SUSP_FRONT_FREQUENCY);
  sensors[SENSOR_FRONT_RIGHT_SUSPENSION_ID] = Sensor("FR Susp", "°", SENSOR_FRONT_RIGHT_SUSPENSION_ID, EK305CAN_ID_ADDRESS_ECU01, EK305CAN_SUSP_FRONT_FREQUENCY);

  //Acelerometros
  // Não é necessário fazer essa parte da programação agora, só quando o filtro de kalman for ser aplicado em versão live
  //acc[EK305CAN_ID_ACC_01];
}
...
void taskErrorsMonitor()
{
  ...
  //ECU 01
  if (EK305CAN_ECU01_ENABLED && ((millis() - tmpRecebimentoAckECU01) >= TMR_CANCHECKERROR))
  {
  ...
  //ECU 02
  if (EK305CAN_ECU02_ENABLED && ((millis() - tmpRecebimentoAckECU02) >= TMR_CANCHECKERROR))
  {
  ...
  //ECU 03
  if (EK305CAN_ECU03_ENABLED && ((millis() - tmpRecebimentoAckECU03) >= TMR_CANCHECKERROR))
  {
  ...
  //ECU 04
  if (EK305CAN_ECU04_ENABLED && ((millis() - tmpRecebimentoAckECU04) >= TMR_CANCHECKERROR))
  {
  ...
  //ECU 15
  if (EK305CAN_ECU15_ENABLED && ((millis() - tmpRecebimentoAckECU15) >= TMR_CANCHECKERROR))
  {
...
void taskCAN()
{
  if (mcp2515.readMessage(&frame) == MCP2515::ERROR_OK) //Recebe o pacote da CAN e checa integridade
  {
    ...
    case EK305CAN_ID_RPM:
      ...
      break;
    case EK305CAN_ID_ACC_01:
      //Acelerômetro 1
      break;
    case EK305CAN_ID_ACC_02:
      //Acelerômetro 2
      break;
    case EK305CAN_ID_ACC_03:
      //Acelerômetro 3
      break;
    case EK305CAN_ID_GYRO_01:
      //Giroscopio 1
      break;
    case EK305CAN_ID_GYRO_02:
      //Giroscopio 2
      break;
    case EK305CAN_ID_GYRO_03:
      //Giroscopio 3
      break;
    case EK305CAN_ID_LAMBA:
      //Lambda
      sensors[SENSOR_LAMBDA_ID].value = frame.data[0];

      setAckTime(sensors[SENSOR_LAMBDA_ID].origin);
      break;
    case EK305CAN_ID_GEAR_POSITION:
      //Gear Position
      sensors[SENSOR_GEAR_ID].value = frame.data[0];

      setAckTime(sensors[SENSOR_GEAR_ID].origin);
      break;
    case EK305CAN_ID_TEMPERATURE:
      //Temperature
      sensors[SENSOR_MOTOR_TEMPERATURE_ID].value = frame.data[1];

      setAckTime(sensors[SENSOR_MOTOR_TEMPERATURE_ID].origin);
      break;
    case EK305CAN_ID_PRESSURE:
      //Pressure
      break;
    case EK305CAN_ID_SUSP_FRONT:
      //SuspFront
    ...
    {
      if (EK305CAN_ECU01_ENABLED)
        CAN_SendACK(&mcp2515, EK305CAN_ID_ADDRESS_ECU01);
      if (EK305CAN_ECU02_ENABLED)
        CAN_SendACK(&mcp2515, EK305CAN_ID_ADDRESS_ECU02);
      if (EK305CAN_ECU03_ENABLED)
        CAN_SendACK(&mcp2515, EK305CAN_ID_ADDRESS_ECU03);
      if (EK305CAN_ECU04_ENABLED)
        CAN_SendACK(&mcp2515, EK305CAN_ID_ADDRESS_ECU04);
      if (EK305CAN_ECU15_ENABLED)
        CAN_SendACK(&mcp2515, EK305CAN_ID_ADDRESS_ECU15);
...
void setAckTime(int origin)
{
  switch (origin)
  {
  case EK305CAN_ID_ADDRESS_ECU01:
    tmpRecebimentoAckECU01 = millis();
    break;
  case EK305CAN_ID_ADDRESS_ECU02:
    tmpRecebimentoAckECU02 = millis();
    break;
  case EK305CAN_ID_ADDRESS_ECU03:
    tmpRecebimentoAckECU03 = millis();
    break;
  case EK305CAN_ID_ADDRESS_ECU04:
    tmpRecebimentoAckECU04 = millis();
    break;
  case EK305CAN_ID_ADDRESS_ECU15:
